...
class COOPSMatcher:
    ...
    def find_nearest(self, lon, lat, max_results=5):
        ...
        nearby = (
            self.coops_stations[self.coops_stations["distance"] <= self.search_radius]
    ...
    def get_best_match(self, lon, lat, require_valid_id=True):
        ...
        if len(matches) == 0:
            msg = (
                f"Warning: No CO-OPS stations found within {self.search_radius} degrees "
                f"of ({lon:.4f}, {lat:.4f})"
            )
            print(msg)
            return None
        ...
        if require_valid_id:
            ...
            if len(matches) == 0:
                msg = (
                    f"Warning: No valid 7-digit CO-OPS stations found "
                    f"within {self.search_radius} degrees"
                )
                print(msg)
                return None
