...
def compare_station(nc_file, station_idx, datum='MSL', search_radius=0.5,
                   plot_file=None, stats_file=None, verbose=False, quiet=False):
    """
    ...
    try:
        ...
        with Fort61Reader(nc_file) as fort61:
            ...
            if not quiet:
                ...
                print(f"Time range: {station_info['time_range'][0]} to {station_info['time_range'][1]}")
            ...
            if not quiet:
                ...
                print(f"Distance: {coops_match['distance']:.4f}° (~{coops_match['distance']*111:.1f} km)")
...
def batch_process_stations(nc_file, station_indices=None, datum='MSL', search_radius=0.5,
                           save_plots=None, verbose=False):
    """
...
def generate_summary_csv(results, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        fieldnames = ['station_idx', 'status', 'station_name', 'coops_station', 'coops_id',
                     'rmse', 'mae', 'bias', 'correlation', 'n_points', 'datum',
                     'coops_distance_km', 'error']
...
Examples:
  ...
  stofs2d-compare fort.61.nc --batch --summary-csv results.csv --html-report report.html --save-plots plots/
...
For more information, see: https://github.com/oceanmodeling/stofs2d-obs
    ...
    station_group.add_argument(
        ...
        type=int,
        metavar='N',
        help='Station index to process (0-based)'
    )
    station_group.add_argument(
        '--list-stations',
        action='store_true',
    ...
    station_group.add_argument(
        '--batch',
        action='store_true',
        help='Batch process multiple stations'
    )
    station_group.add_argument(
        '--station-range',
        nargs=2,
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='FILE',
        help='Save batch summary to CSV file'
    )
    parser.add_argument(
        '--html-report',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='FILE',
        help='Save comparison plot to file (PNG, PDF, SVG)'
    )
    parser.add_argument(
        '--stats-file',
        type=str,
        metavar='FILE',
        help='Save statistics to file (CSV or JSON)'
    )
