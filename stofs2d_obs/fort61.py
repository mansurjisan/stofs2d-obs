...
class Fort61Reader:
    ...
    def get_station_info(self, station_idx):
        ...
        name = "".join(
            ...
            for c in self.station_names_var[station_idx]
        ).strip()
    ...
    def get_station_data(self, station_idx):
        ...
        # Create DataFrame
        df = pd.DataFrame({"time": valid_times, "water_level": valid_zeta})
    ...
    def list_stations(self, n=10):
        ...
        for i in range(min(n, self.n_stations)):
            ...
            print(
                f"{info['index']:<8} {info['name']:<30} {info['lon']:<12.6f} {info['lat']:<12.6f}"
            )
